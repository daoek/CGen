#!/usr/bin/env sh
#
# Installs CGen into a per-user directory (default $HOME/.local/share/CGen) on Linux or macOS.
...
REPO_SLUG="daoek/CGen"
INSTALL_DIR="${CGEN_INSTALL_DIR:-$HOME/.local/share/CGen}"
VERSION=""
SKIP_BUILD=0
MARKER_NAME=".cgen-install-marker"
...
if [ -d "$INSTALL_DIR" ] && [ -n "$(ls -A "$INSTALL_DIR" 2>/dev/null)" ] && [ ! -f "$INSTALL_DIR/$MARKER_NAME" ]; then
    echo "Refusing to overwrite non-CGen directory: $INSTALL_DIR" >&2
    exit 1
fi
...
if [ -n "$VERSION" ]; then
    ...
    bare_version=${tag#v}
    jar_name="cgen-$bare_version.jar"
    ...
    download_dir=$(mktemp -d "${TMPDIR:-/tmp}/cgen-install.XXXXXX")
    ...
    if ! curl -fsSL -o "$download_dir/CGen.sh" "$raw_base_url/CGen.sh" \
    ...
    launcher_source="$download_dir/CGen.sh"
    ...
    source_jar=$(find "$REPO_ROOT/target" -maxdepth 1 -name 'cgen-*.jar' \
        ! -name 'original-*' ! -name '*-sources.jar' ! -name '*-javadoc.jar' -print \
        | sort | tail -n 1)
    if [ -z "$source_jar" ]; then
        echo "No packaged CGen JAR found. Run without --skip-build first." >&2
        exit 1
    fi
    launcher_source="$SCRIPT_DIR/CGen.sh"
    uninstall_source="$SCRIPT_DIR/uninstall.sh"
fi

mkdir -p "$INSTALL_DIR"
cp "$source_jar" "$INSTALL_DIR/cgen.jar.new"
mv -f "$INSTALL_DIR/cgen.jar.new" "$INSTALL_DIR/cgen.jar"
cp "$launcher_source" "$INSTALL_DIR/CGen"
chmod +x "$INSTALL_DIR/CGen"
cp "$uninstall_source" "$INSTALL_DIR/uninstall.sh"
chmod +x "$INSTALL_DIR/uninstall.sh"
echo "CGen managed installation. Safe removal requires this marker." > "$INSTALL_DIR/$MARKER_NAME"

installed_hash=$(sha256_of "$INSTALL_DIR/cgen.jar")
echo "CGen installed in $INSTALL_DIR"
echo "Installed jar SHA256: $installed_hash"
echo "To uninstall later: $INSTALL_DIR/uninstall.sh"

if ! command -v java >/dev/null 2>&1; then
    echo >&2
    echo "Warning: no 'java' found on PATH. CGen needs a Java 17+ runtime to RUN (installing it" >&2
    echo "needed no Java at all) - install one, e.g. from https://adoptium.net, before using CGen." >&2
fi
