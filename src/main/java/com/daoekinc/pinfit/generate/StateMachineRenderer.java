...
final class StateMachineRenderer {
    ...
    private static void appendHeaderTop(StringBuilder out, StateMachineSpec machine, DocumentationRenderer docs,
        ...
        out.append(PinfitTag.generatedFile("state-machine-header", machine.source().getFileName().toString())).append('\n');
    ...
    private static void appendCoreFunctionDeclarations(StringBuilder out, ProjectConfig project, StateMachineSpec machine) {
        ...
        out.append(PinfitTag.generatedItem("function", goToState)).append('\n');
    ...
    private static void appendSourceTop(StringBuilder out, StateMachineSpec machine, DocumentationRenderer docs, UserRegions user) {
        out.append(PinfitTag.generatedFile("state-machine-source", machine.source().getFileName().toString())).append('\n');
    ...
    private static void appendEntryExitFunctions(StringBuilder out, ProjectConfig project, StateMachineSpec machine, UserRegions user) {
        for (StateMachineSpec.State state : machine.states()) {
            ...
            out.append(PinfitTag.generatedItem("private-function", enter)).append('\n');
            ...
            out.append(PinfitTag.generatedItem("private-function", exit)).append('\n');
    ...
    private static void appendEventFunctions(StringBuilder out, ProjectConfig project, StateMachineSpec machine, UserRegions user) {
        for (StateMachineSpec.Event event : machine.events()) {
            ...
            appendParameters(out, event.parameters(), true);
            out.append(")\n{\n");
            out.append(indent(project, 1)).append("bool pinfit_transitioned = false;\n");
            ...
            out.append(indent(project, 2)).append("default:\n").append(indent(project, 3)).append("break;\n");
            out.append(indent(project, 1)).append("}\n\n");
            out.append(indent(project, 1)).append("if (!pinfit_transitioned)\n").append(indent(project, 1)).append("{\n");
    ...
    private static void appendTransitionCase(StringBuilder out, ProjectConfig project, StateMachineSpec machine,
        ...
        int bodyLevel = 3;
        if (transition.guard()) {
            out.append(indent(project, 3)).append("bool pinfit_guard = true;\n\n");
            out.append(user.render("transition." + regionKey + ".guard", "", indent(project, 3)));
            out.append(indent(project, 3)).append("if (pinfit_guard)\n").append(indent(project, 3)).append("{\n");
            bodyLevel = 4;
        }
        ...
        out.append(indent(project, bodyLevel)).append("pinfit_transitioned = true;\n");
        if (transition.guard()) {
