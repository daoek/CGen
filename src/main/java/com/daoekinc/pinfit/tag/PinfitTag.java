...
public final class PinfitTag {
    ...
    // Every read-side pattern accepts both the current "Pinfit" brand and the legacy "CGen" one
    // (the project's previous name), so a file generated by any past version keeps working
    // untouched. Every write-side method below emits "Pinfit" only - a legacy marker is silently
    // upgraded to the current brand the next time its file is regenerated.
    private static final Pattern MARKER = Pattern.compile("^\\s*(?:'\\s*)?/\\*@(?:Pinfit|CGen)\\(([^)]*)\\)\\*/\\s*$");
    private static final Pattern USER_BEGIN = Pattern.compile("^\\s*/\\*@(?:Pinfit|CGen) usercode\\+ (\\S+)\\*/\\s*$");
    private static final Pattern USER_END = Pattern.compile("^\\s*/\\*@(?:Pinfit|CGen) usercode-\\*/\\s*$");
    // The pre-migration user-region syntax ("/*@CGen(+name)*/ ... /*@CGen(-name)*/"), replaced
    // project-wide by usercode+/usercode- before CGen 1.0 - i.e. before the project was ever
    // called Pinfit, so this old syntax only ever existed under the "CGen" brand.
    // No current renderer emits this, but a file written by that old version and never
    // regenerated since would still have it on disk - see TagHelper's use of these, which
    // refuses to touch such a file rather than silently discarding what's inside it (extract()
    // otherwise treats an unrecognized marker line as plain body text, i.e. as if the region
    // never existed).
    ...
    private static final Pattern SKELETON_HASH = Pattern.compile("^\\s*(?:'\\s*)?/\\*@(?:Pinfit|CGen)\\(skeleton-hash:([0-9a-fA-F]+)\\)\\*/\\s*$");

    private PinfitTag() {
    }

    public static String generatedFile(String kind, String source) {
        return "/*@Pinfit(file:" + kind + ":" + source + ")*/";
    }

    public static String generatedItem(String item, String name) {
        return "/*@Pinfit(" + item + ":" + name + ")*/";
    }

    public static String userBegin(String name) {
        return "/*@Pinfit usercode+ " + name + "*/";
    }

    public static String userEnd() {
        return "/*@Pinfit usercode-*/";
    }
    ...
    public static String skeletonHash(String hash) {
        return "/*@Pinfit(skeleton-hash:" + hash + ")*/";
    }

    /** The hash from a "/*@Pinfit(skeleton-hash:...)*&#47;" line, or null if the line isn't one. */
    public static String skeletonHashValue(String line) {
        Matcher matcher = SKELETON_HASH.matcher(line);
        return matcher.matches() ? matcher.group(1) : null;
    }

    /** The region name from an "/*@Pinfit(orphaned-user-region:name)*&#47;" line, or null. */
