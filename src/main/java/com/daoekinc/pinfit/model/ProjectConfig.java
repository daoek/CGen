...
public record ProjectConfig(
    ...
    public static ProjectConfig from(Path file, Map<String, Object> yaml) {
        ...
        if (schema != 1) {
            throw new PinfitException(context + " uses unsupported schema " + schema);
        }
        Path root;
        try {
            ...
            // in PinfitGenerator.generate()) uses the same representation - on Windows, a raw
            ...
            throw new PinfitException("Cannot resolve project root for " + file + ": " + exception.getMessage(), exception);
        }
        ...
        if (!style.equals("doxygen") && !style.equals("none") && !style.equals("custom")) {
            throw new PinfitException("documentation.style must be doxygen, none, or custom");
        }
        ...
        if (style.equals("custom") && customFile == null) {
            throw new PinfitException("documentation.file is required when documentation.style is custom");
        }
        ...
        if (indent < 2 || indent > 8) {
            throw new PinfitException("format.indent must be between 2 and 8");
        }
        ...
        String lineEnding = switch (lineEndingName) {
            case "lf" -> "\n";
            case "crlf" -> "\r\n";
            default -> throw new PinfitException("format.lineEnding must be lf or crlf");
        };
        ...
        if (!publicVariableStyle.equals("extern") && !publicVariableStyle.equals("accessors")) {
            throw new PinfitException("format.publicVariables must be extern or accessors");
        }
        ...
        if (!functionNaming.equals("snake_case") && !functionNaming.equals("camelCase")) {
            throw new PinfitException("format.functionNaming must be snake_case or camelCase");
        }
    ...
    private static Path resolveInside(Path root, String configured, String label) {
        ...
        try {
            ...
            throw new PinfitException(label + " is not a valid path", exception);
        }
        if (relative.isAbsolute()) {
            throw new PinfitException(label + " must be relative to the project");
        }
        Path resolved = root.resolve(relative).normalize();
        if (!resolved.startsWith(root)) {
            throw new PinfitException(label + " must stay inside the project");
        }
        return resolved;
    }
}
