...
/**
 * Finds function definitions a user typed directly inside a usercode region of a CGen-generated
 * module source (.c) file - not part of the YAML spec, so CGen never emitted a prototype for them -
...
public final class PrototypeScanner {
    ...
    public static Scan scan(String content, String displayPath) {
        ...
        if (lines.stream().noneMatch(CGenTag::isGeneratedFile)) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            if (CGenTag.userBeginName(line) != null) {
                depth++;
                continue;
            }
            if (CGenTag.isUserEnd(line)) {
    ...
    private static int findInsertionLine(List<String> lines) {
        for (int i = 0; i < lines.size(); i++) {
            if (TARGET_REGION.equals(CGenTag.userBeginName(lines.get(i)))) {
                for (int j = i + 1; j < lines.size(); j++) {
                    if (CGenTag.isUserEnd(lines.get(j))) {
                        return j;
                    }
                }
            }
