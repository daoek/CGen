...
## @CGenSwitch: filling in a switch statement by hand

Inside any usercode region - a module function body, a contract
implementation, a state-machine event body, anywhere CGen preserves your
code - you can write a `@CGenSwitch <enum_type>` comment directly above your
own `switch` statement instead of writing out every `case` yourself:
...
On the next `generate`, CGen resolves `flash_opcodes_t` (first checking every
`enums:` block declared anywhere in the project's `.module.yaml`/
`.interface.yaml` files; if it's not declared in YAML, CGen looks for a
matching `typedef enum { ... } flash_opcodes_t;` in the project's `.h`/`.c`
files and asks you to confirm it found the right one before using it), then
rewrites the switch with one `case` per member plus a `default`, each with
its own nested user region:
...
From then on, only edit inside those nested `switchcase.<enum>.<case>`
regions - like any other user region, they survive regeneration, and a new
enum member gets its own empty case added automatically. The first time you
tag an existing hand-written switch, any code already inside a `case` whose
label matches a member of the enum is migrated into its new region rather
than discarded; a `case` whose label does *not* match any member is left
alone and generation fails with a clear error instead of silently dropping
it, so you can fix or remove it first. Anything you write in the switch body
outside a `case`'s own region (a stray statement, a hand-added case you
didn't run `generate` on yet) is not preserved - the skeleton around the
cases is CGen's from that point on, same as everywhere else CGen generates
structure around a user region.

The `.h`/`.c` fallback scan is a light, best-effort parser (a single
`typedef enum { ... } name;` match, members split on commas) - not a real C
preprocessor. Two files defining the same enum name differently, or a body
using macros for values, are not handled; declare the enum in YAML `enums:`
instead if you hit that.

Once you confirm an enum found this way, CGen records *where* it's declared -
never a copy of it. It adds a `{ name: flash_opcodes_t, file: ... }` entry to
the `externalEnums:` block of the `.module.yaml` that owns the file the
`@CGenSwitch` is in, `file` a path relative to that YAML file:
...
This is a link, not a declaration: unlike `enums:`, CGen never emits its own
`typedef` for an `externalEnums:` entry, since the type already exists in
`file` and redeclaring it would conflict with - or duplicate - that real
definition. Later generates read `file` directly to get the case list, no
project scan or re-asking needed; the entry is a plain text insertion, same
as `enums:` gets (rest of the file, comments included, is untouched; verified
by re-parsing, rolled back if that somehow fails). This only works from a
module owner, since only `.module.yaml` supports `externalEnums:`; from a
state-machine, observer, command-table, or adapter file, confirmation is
asked every time.

## `--also-nested`

`generate` normally leaves a subdirectory with its own `cgen.yaml` alone -
that's a separate, self-contained nested project (e.g. an imported library),
generated by running CGen from inside it. Pass `--also-nested` to also
generate every nested project found anywhere under the scanned directory in
the same run (nested-inside-nested included), each using its own `cgen.yaml`
settings, not the outer project's:
